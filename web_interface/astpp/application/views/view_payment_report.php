...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>adminReports/pyament_report_list/",
    method: 'GET',
    dataType: 'json',
	colModel : [
		{display: 'Account', name:'Number', width:120,  sortable: false, align: 'center'},
...
// 		{display: 'Recharge By', name: 'RechargeBY',width:100, sortable: false, align: 'center'},			
	...
	$.ajax({
	  type:'POST',
	  url: '<?=base_url()?>adminReports/payment_search',
    ...
    $("#id_reset").click(function(){
	    $.ajax({url:'<?=base_url()?>adminReports/clearsearchfilter_payment/',
	    success:function(){
		$('#flex1').flexReload(); }
	    });
    });
		  ...
		  <input type="hidden" name="advance_search" value="1">
			...
			<fieldset >
			  ...
			  <li>
				  ...
				  <span>
				    <label>Account:</label>
				    <input size="20" class="text field" name="account"> &nbsp;
				    <select name="account_operator" class="field select ">
