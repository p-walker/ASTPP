...
function selected_routes(route_id){
	...
	$.ajax({
	    ...
	    async    : true, 
	    data:"prefixes="+text,  
	    url      : "/rates/set_selected_prefixes",
...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>rates/Package_routes_grid/<?=$package_id?>",
    method: 'GET',
    dataType: 'json',
	colModel : [
...
$("#rates_routes_search").click(function(){
	$.ajax({type:'POST', url: '<?=base_url()?>rates/Package_routes_search', data:$('#search_form12').serialize(), success: function(response) {
	      $('#flex1').flexReload();
	    }});
	});
	
	$("#id_reset").click(function(){
		$.ajax({url:'<?=base_url()?>rates/Package_clearsearchfilter_routes/', success: function(){
		$('#flex1').flexReload(); }
		});
	});
	...
	$("#add_patterns_btn").click(function(){
	  if(document.getElementById("add_patterns").value !=""){
		...
		url      : "/rates/add_pckg_pattern/<?=$package_id?>",
		success: function(data){
		    //window.close();
