...
class Accounts extends CI_Controller {
	...
	$data['payment_type']= $this->config->item("payment_type");
		...
		$_POST["payment_for"] = $accountinfo["accountid"];
                if ($logintype == 1 || $logintype == 5) {
                    
                    if ($accountinfo['reseller'] == $username) {
                        $response = $this->accounts_model->account_process_payment($_POST);
...
// 	$data['payment_type']= $this->config->item("payment_type");
    ...
    function edit($account_number = "") {
        ...
        if (!empty($_POST)) {
            ...
            if ($validation_result === true) {
                $_POST['credit_limit'] = $this->common_model->add_calculate_currency($_POST['credit_limit'], '', '', false, false);
                $response = $this->accounts_model->edit_account($_POST);                
                $this->common_model->status_message($response);
		  //$this->session->set_userdata('astpp_notification', 'Account Edited Sucessfully!');
                if ($this->input->post('accounttype') == 3) {
                    redirect(base_url() . 'lcr/providers/');
                } else {                    
                    redirect(base_url().'accounts/account_detail/'.$validate_data['customnum']);
                }
            } else {
                $data['account'] = $_POST;
                $this->session->set_userdata('astpp_errormsg', $validation_result);
...
//                             $this->common_model->calculate_currency($this->Astpp_common->accountbalance($row['number'])),
    ...
    function get_action_buttons($id, $accountid='') {                
        ...
        $ret_url = '';
        $ret_url .= '<a href="' . base_url() . 'accounts/payment_process/' . $id . '/" class="icon payment_image" rel="facebox" title="Process Payment">&nbsp;</a>&nbsp;';
        $ret_url .= '<a href="' . base_url() . 'accounts/account_detail/' . $id . '/" class="icon details_image" title="View Details">&nbsp;</a>&nbsp;';
//         $ret_url .= '<a href="' . base_url() . 'accounts/edit/' . $id . '/" class="icon edit_image" title="Edit Account">&nbsp;</a>';
        $ret_url .= '<a href="' . base_url() . 'accounting/account_taxes/edit/' . $accountid . '/" class="icon tax_image" title="Add Account Taxes" rel="facebox" >&nbsp;</a>';
        $ret_url .= '<a href="' . base_url() . 'accounts/add_callerid/' . $accountid . '/" class="icon callerid_image" title="Add CallerID" rel="facebox" >&nbsp;</a>';

        $ret_url .= '<a href="' . base_url() . 'accounts/remove/' . $id . '/" class="icon delete_image" title="Remove Account" onClick="return get_alert_msg();">&nbsp;</a>';
...
// 		}
    ...
    function get_action_buttons_invoice_download($id) {
        ...
        $ret_url = '';
        $ret_url .= '<a href="' . base_url() . 'accounts/download_invoice/' . $id . '/" class="icon details_image" title="Download Invoice">&nbsp;</a>';
        return $ret_url;
    }
    ...
    function get_action_buttons_invoice($id) {
        $viewdetails_style = 'style="text-decoration:none;background-image:url(/images/details.png);"';
        $ret_url = '';
        $ret_url .= '<a href="' . base_url() . 'accounts/view_invoice/' . $id . '/" class="icon details_image" title="View Invoice">&nbsp;</a>';
        return $ret_url;
    }
...
//                         $row['created_date'],
    ...
    function get_action_buttons_chargelist($action, $id, $username) {
        ...
        $ret_url .= '<a href="/accounts/remove_account_details/' . $action . '/' . $id . '/' . $username . '" class="icon delete_image" title="Delete" onClick="return get_alert_msg();">&nbsp;</a>';
    ...
    /**
     * -------Here we write code for controller accounts functions block_prefix_json------
     * Listing of Blocked prefixes detail through account no
     * @account_number: Account Number
     */
    function block_prefix_json($account_number=NULL) {

        $accountinfo = $this->accounts_model->get_account_by_number(urldecode($account_number));
        //$accountinfo = $this->session->userdata('accountinfo');

        $json_data = array();
        $count_all = $this->accounts_model->list_block_prefix_count($accountinfo['accountid']);
        ...
        $query = $this->accounts_model->list_blocked_prefixes($accountinfo['accountid'], $start, $perpage);
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                ...
                $json_data['rows'][] = array('cell' => array(
					      ...
					      $row['blocked_patterns'],
					      $this->get_action_buttons_blocklist($row['id'],$account_number)));
            }
        }
        echo json_encode($json_data);
    }
    function get_action_buttons_blocklist($id,$account_number) {
        $delete_style = 'style="text-decoration:none;background-image:url(/images/delete.png);"';
        $ret_url = '';
        $ret_url .= '<a href="/accounts/remove_block_details/'.$id.'/'.$account_number.'" class="icon delete_image" title="Delete" onClick="return get_alert_msg();">&nbsp;</a>';

        return $ret_url;
    }

    function remove_block_details($block_id,$accountnum){
	$this->db->where("id",$block_id);
	$this->db->delete("block_patterns");
	redirect(base_url().'accounts/account_detail/'.$accountnum);
    }
    function add_block_prefixes($account_num){
	$data['app_name'] = 'ASTPP - Open Source Billing Solution | Accounts | Block Prefix List';
        $data['username'] = $this->session->userdata('user_name');
        $data['page_title'] = 'Block Prefix';
        $data['cur_menu_no'] = 1;
    ...
    function routes_grid($accountnum)
    {
	$json_data = array();
	$accountinfo = $this->accounts_model->get_account_by_number(urldecode($accountnum));
	$count_all = $this->accounts_model->getRoutesList(false,$accountinfo['accountid'],"","");
	...
	$json_data['total'] = ($config['total_rows']>0) ? $config['total_rows'] : 0;					
	  ...
	  $start = 0;
	  $selected_arr = array();
	$query = $this->accounts_model->getRoutesList(true,$accountinfo['accountid'],$start, $perpage);
	if($query->num_rows() > 0)
	{
    ...
    function insert_block_prefix($account_id){
    
      $accountinfo = $this->accounts_model->get_account_by_number(urldecode($account_id));
      $result = $this->accounts_model->insert_block(rtrim($_POST['add_patterns'],","),$accountinfo['accountid']);
      $this->session->set_userdata("selected_prefixes","");
    }
    function insert_block_pattern($account_id){
      $accountinfo = $this->accounts_model->get_account_by_number(urldecode($account_id));
      $count_all = $this->accounts_model->list_block_prefix_count($accountinfo['accountid'],$_POST['add_to_list']);
      if($count_all > 0){
	echo 0;
      }else{
	$tmp = array("accountid" =>$accountinfo['accountid'],
		    "blocked_patterns" =>$_POST['add_to_list']);
	$result = $this->accounts_model->insert_block_patern($tmp);
	echo 1;
      }
    }
    ...
    function delete_selected_prefixes(){
      $delete_result = $this->accounts_model->selected_prefixes_delete($_POST['deletable_id']);
      if($delete_result){
	echo "1";
      }else{
	echo "0";
